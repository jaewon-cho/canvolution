...
#' evolution_mut_exp_cellwise_sample_wise
#'
#' Correlation analysis between each evolution path and mutated gene expression
#'
#' @param meta_info seurat_obj@meta.data
#' @param name title for your data
#' @param pat_info variable name for patient in the "meta_info"
...
#' @param cell_cnt_thr threshold for excluding clones less than the given cell count
#'
#' @return Correlation analysis between each evolution path and mutated gene expression
#'
#' @examples
#' #meta_info <- seurat_obj@meta.data
#' meta_info <- read.table("data/meta_info_lung", sep = "\t", check.names = F)
...
#' clone_path_file_prefix <- "data/robustclone/"
#' feature_file_prefix <- "data/feature/"
#' #please run mut_gmt_module and save in the right directory
#' output_file_prefix <- "./"
...
#'
#' @export
evolution_mut_exp_cellwise_samplewise <- function(meta_info, name, pat_info, sample_info, clone_path_file_prefix, feature_file_prefix, output_file_prefix, cell_cnt_thr=10){
        sample_list <- as.character(meta_info[,sample_info])
        sample_list_uniq <- unique(sample_list)
        ...
        for(j in c(1:length(sample_pat_map))){
                ...
                a2 <- read.delim(paste(feature_file_prefix, name, "_mut_gmt_addmodule", sep = ""),sep = "\t", check.names = F)
                ...
                write.table(total_path_scc, paste(output_file_prefix, name, "_", sample, "_clone_evolution_mut_exp_scc_sample", sep = ""), quote = F, sep = "\t")
...
#print(m)
                ...
                write.table(total_path_scc, paste(output_file_prefix, name, "_", sample, "_clone_abundance_mut_exp_scc_sample", sep = ""), quote = F, sep = "\t")
